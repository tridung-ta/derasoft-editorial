...
include_once(ROOT_PATH."classes/dao/orderloginfo.class.php");
...
class OrderLogs extends Model {
	...
	function __construct($store_id = 0, $database = '') {
		...
		$this->table = DB_PREFIX."order_log";
...
*-----------------------------------------------------------------------*/
	function getObject($value = '0', $key = 'id') {
		...
		if($result) {
			$object = new OrderLogInfo
						(	$result[0]['properties'],$result[0]['store_id'],
						 	$result[0]['order_id'],
							$result[0]['user_id'],
							$result[0]['action'],
							...
							$result[0]['ip'],
							$result[0]['status'],
							$result[0]['internal'],    // them internal
							$result[0]['id']
						);
			return $object;
		}
	...
	function getObjects($page = 1, $condition = '`id` = 0', $sort = array(), $items_per_page = DEFAULT_ADMIN_ROWS_PER_PAGE) {
		...
		if($results) {
			...
			foreach($results as $key => $result) {
				$objects[] = new OrderLogInfo
								(	$result['properties'],$result['store_id'],
									$result['order_id'],
									$result['user_id'],
									$result['action'],
									$result['date_created'],
