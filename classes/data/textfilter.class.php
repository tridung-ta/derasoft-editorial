...
class TextFilter  
{
	...
	function __construct( $removeJavaScript = true, $htmlAllowedTags = true )
	{
		$this->removeJavaScript = $removeJavaScript;
		$this->htmlAllowedTags = $htmlAllowedTags;
	}

        function convert_vi_to_en($str) {
            $str = str_replace(
                array('à', 'á', 'ả', 'ã', 'ạ', 'ă', 'ằ', 'ắ', 'ẳ', 'ẵ', 'ặ', 'â', 'ầ', 'ấ', 'ẩ', 'ẫ', 'ậ', 'đ', 'è', 'é', 'ẻ', 'ẽ', 'ẹ', 'ê', 'ề', 'ế', 'ể', 'ễ', 'ệ', 'ì', 'í', 'ỉ', 'ĩ', 'ị', 'ò', 'ó', 'ỏ', 'õ', 'ọ', 'ô', 'ồ', 'ố', 'ổ', 'ỗ', 'ộ', 'ơ', 'ờ', 'ớ', 'ở', 'ỡ', 'ợ', 'ù', 'ú', 'ủ', 'ũ', 'ụ', 'ư', 'ừ', 'ứ', 'ử', 'ữ', 'ự', 'ỳ', 'ý', 'ỷ', 'ỹ', 'ỵ'),
...
*/
	...
	function htmlDecode( $htmlString, $quote_style = ENT_QUOTES )
         ...
         */
        function urlize( $string, $domainize = false, $separator = "_" )
        {
...
#End edited by Mai Minh for Vietnamese
        ...
        function cleanVietnamese( $string )
        {
            // remove unnecessary spaces and make everything lower case
		    $string = preg_replace( "/ +/", " ", strtolower($string) );
