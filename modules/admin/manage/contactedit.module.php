...
// Lấy id liên hệ cần edit
$id = $request->element('id');
$item = $contacts->getObject($id);
if (!$item) {
    header('location:/' . ADMIN_SCRIPT . "?op=manage&act=contact&mod=list&rcode=7"); // record not found
    exit;
}
...
if ($_POST && $request->element('doo') == 'submit') {
    ...
    if ($validate['invalid']) {
        ...
        // Cập nhật dữ liệu thay vì thêm mới
        $result = $contacts->updateData($data, $id);
...
function validateData($request) {
    ...
    if ($error['INPUT']['name']['error'] || $error['INPUT']['link_map']['error'] ||
        $error['INPUT']['phone_number']['error'] || $error['INPUT']['address']['error']) {
        $error['invalid'] = 1;
    }
