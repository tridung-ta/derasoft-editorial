...
# Page navigation
$rowsPages = $units->getNumItems('id', $pages_condition, $items_per_page);
$template->assign('rowsPages', $rowsPages);
if ($page < 1) $page = 1;
if ($page > $rowsPages['pages']) $page = $rowsPages['pages'];
$start_num = ($page - 1) * $items_per_page + 1;
$template->assign('startNum', $start_num);
$url = '/' . ADMIN_SCRIPT . "?op=system&act=master&mod=unitlist&doo=$do&kw=".urlencode($kw)."&filter_status=$filter_status&filter_date_created=$filter_date_created&filter_type=$filter_type&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&pId=$pId&module=$module&pg=%d";
$urls = new Url();
$pager = $urls->genPager($url, $rowsPages['pages'], $page);
$template->assign('pager', $pager);

# Get objects
$listItems = $units->getObjects($page, $condition, $sort, $items_per_page);
...
# Link
$link = '/' . ADMIN_SCRIPT . "?op=system&act=master&mod=unitlist&kw=".urlencode($kw)."&filter_status=$filter_status&filter_date_created=$filter_date_created&filter_type=$filter_type&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&pId=$pId&module=$module&pg=$page";
...
if ($_POST) {
	switch ($do) {
		case 'enable':
			...
			if ($id) {
				$units->changeStatus($id, S_ENABLED);
				$result_code = 1;
				# Operation tracking
				$trackings->addData(array('store_id' => $storeId, 'username' => $userInfo->getUsername(), 'action' => sprintf($amessages['tracking']['enable_unit'], $units->getNameFromId($id)), 'date_created' => date("Y-m-d H:i:s"), 'ip' => $_SERVER['REMOTE_ADDR']));
			} else {
				...
				if ($ids) {
					...
					foreach ($ids as $id) {
						$units->changeStatus($id, S_ENABLED);
						$listArticle .= ($listArticle ? ',&nbsp;' : '') . $units->getNameFromId($id);
					}
					$result_code = 1;
					# Operation tracking
		...
		case 'disable':
			$id = $request->element('id');
			if ($id) {
				$units->changeStatus($id, S_DISABLED);
				$result_code = 2;
				# Operation tracking
				$trackings->addData(array('store_id' => $storeId, 'username' => $userInfo->getUsername(), 'action' => sprintf($amessages['tracking']['disable_unit'], $units->getNameFromId($id)), 'date_created' => date("Y-m-d H:i:s"), 'ip' => $_SERVER['REMOTE_ADDR']));
			} else {
				$ids = $request->element('ids');
				if ($ids) {
					$listArticle = '';
		...
		case 'delete':
			$id = $request->element('id');
			if ($id) {
				$units->changeStatus($id, S_DELETED);
				$result_code = 3;
				# Operation tracking
				$trackings->addData(array('store_id' => $storeId, 'username' => $userInfo->getUsername(), 'action' => sprintf($amessages['tracking']['delete_unit'], $units->getNameFromId($id)), 'date_created' => date("Y-m-d H:i:s"), 'ip' => $_SERVER['REMOTE_ADDR']));
			} else {
				$ids = $request->element('ids');
				if ($ids) {
					$listArticle = '';
		...
		case 'changeposition':
			...
			if ($positions) {
				foreach ($positions as $key => $value) {
					$units->changePosition($key, $value);
				}
				$result_code = 4;
				# Operation tracking
		...
		case 'cleantrash':
			...
			break;
		case 'cancel':
			header('location:' . '/' . ADMIN_SCRIPT . "?op=system&act=master&mod=unitlist&lang=$lang&ecode=7&pId=$pId");
			exit;
			break;
	}
	header('location:' . '/' . ADMIN_SCRIPT . "?op=system&act=master&mod=unitlist&doo=$do&kw=".urlencode($kw)."&filter_status=$filter_status&filter_date_created=$filter_date_created&filter_type=$filter_type&lang=$lang&ipp=$items_per_page&sk=$sort_key&sd=$sort_direction&pg=$page&ecode=$error_code&rcode=$result_code&pId=$pId");
} else {
}
