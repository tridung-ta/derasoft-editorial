...
$fieldList = $fields->getObjects(1, "`status`='1' AND `module`='carcompany'", array('position' => 'ASC'),99);
if ($fieldList) $template->assign('fieldList', $fieldList);

if ($_POST) { # if form is submitted
    if ($request->element('doo') == 'cancel') {    # Cancel
        header('location:' . '/' . ADMIN_SCRIPT . "?op=system&act=config&mod=carcompany&lang=$lang&ecode=7");
        exit;
    }
    if ($request->element('doo') == 'submit') {
        ...
        if ($validate['invalid']) {    # data input is not in valid form
            ...
            if (!$validate['invalid']) {
				...
				# Operation tracking
				$trackings->addData(array('store_id' => $storeId, 'username' => $userInfo->getUsername(), 'action' => $amessages['carcompany'], 'date_created' => date("Y-m-d H:i:s"), 'ip' => $_SERVER['REMOTE_ADDR']));

				# Redirect to editing page
				header('location:' . '/' . ADMIN_SCRIPT . "?op=system&act=config&mod=carcompany&lang=$lang&rcode=7");
            }
        }
