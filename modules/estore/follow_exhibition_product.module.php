...
$templateFile = 'follow_exhibition_product.tpl.html';
...
$isFollowExhibitionProduct = 1;
$template->assign('isFollowExhibitionProduct', $isFollowExhibitionProduct);
...
$navConfig = [
    'vn' => [
        ...
        ['name' => 'Triển lãm',    'url' => "/san-pham-trien-lam"],
        $currentNav,
    ],
    ...
    'en' => [
        ...
        ['name' => 'Exhibition Products', 'url' => '/en/exhibition-products'],
        $currentNav,
    ],
    ...
    'zh' => [
        ...
        ['name' => '展览产品', 'url' => '/zh/exhibition-products'],
        $currentNav,
    ],
];
...
// Products list
// (categoryId ép về int trước khi nối vào SQL để tránh injection)
...
if ($categoryId == 184) {
    $result = paginate(
        $request,
        $products,
        "status = 1 AND (category_id = 186 OR category_id = 187)",
        "p.status = 1 AND (p.category_id = 186 OR p.category_id = 187)",
        ['p.id' => 'DESC'],
        6
    );
} else {
    $result = paginate(
        ...
        "p.status = 1 AND p.category_id = $categoryId",
        ['p.id' => 'DESC'],
        6
    );
}

# Filter cate con của "Thư viện thiết kế sản phẩm"
$childOfCategory = $menus->getObjects(1, "parent_id = 169 AND parent_id != 0 AND `status` = '1'", [], 999);
...
# Danh sách cate con của sản phẩm triển lãm
$childCate = $menus->getObjects(1, "parent_id = 169 AND `status` = '1'", [], 999);

$cate170 = $menus->getObject(170); # Cate hộp đèn quảng cáo
if ($cate170) {
    $childCate[] = $cate170;
}
