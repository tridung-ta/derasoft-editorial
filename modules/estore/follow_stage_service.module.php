...
$templateFile = 'follow_stage_service.tpl.html';
...
$caseStudy = $articles->getObjects(
    1,
    "a.`status`='1' AND a.`category_id`=77",
    ['COALESCE(a.`publish_at`, a.`date_created`)' => 'DESC'],
    10
);
$template->assign('caseStudy', $caseStudy);

$isShowBlockService = in_array($articleCategoryObj->getId(), [83]);
...
# SEO
if ($lang == 'en') {
    $pageTitle = $menuObj->getProperty('custom_en_titleSeo')?: $menuObj->getProperty('custom_titleSeo')?: $menuObj->name;
    $pageKeywords = $menuObj->getProperty('custom_en_meta_keyword')?: $menuObj->getProperty('custom_meta_keyword');
    $pageDescription = $menuObj->getProperty('custom_en_intro')?: $menuObj->getProperty('custom_intro');
} else {
