...
function decodeLibraryRows($rows){foreach($rows as &$row){$data=json_decode($row['payload'],true);$row['data']=is_array($data)?$data:array();}unset($row);return $rows;}
$template->assign('savedItems',decodeLibraryRows($library->getItems($_SESSION['store_customerId'],'saved',100)));
$template->assign('historyItems',decodeLibraryRows($library->getItems($_SESSION['store_customerId'],'history',100)));
